...
const HeroScene = lazy(() => import("@/components/HeroScene.lazy"));
...
function DeferredHeroScene() {
  ...
  const fallback = <div className="grid size-full place-items-center"><Cpu className="size-20 animate-pulse text-brand-cyan" /></div>;
  if (!show) return fallback;
  return <Suspense fallback={fallback}><HeroScene /></Suspense>;
}
